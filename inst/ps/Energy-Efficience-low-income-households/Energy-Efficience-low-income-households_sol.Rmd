...
setwd("C:/Users/janna/Desktop/Masterarbeit")
#set the name of the problem set
ps.name = "Energy-Efficience-low-income-households"; sol.file = paste0(ps.name,"_sol.Rmd")

#set required libraries
libs = c("ggplot2", "ggthemes", "haven", "dplyr", "skimr", "ggbreak", "patchwork", "lfe", "stargazer", "AER")
...
#< quiz "energy cost attention"
question: In case of rational behavior, would high-income or low-income households pay more attention to energy costs?

sc:
- High-income households.
- Low-income households.*
success: Great, your answer is correct!
failure: Sorry that is wrong. Try again.
...
#< quiz "poverty"
question: Guess how much of the Kenyan population approximately live below the poverty line of 3.20 US Dollar.

sc:
- less than 20 %
- 30-50 %
- 60-80 % *
success: Great, your answer is correct. 2015 it was 63.7 %, but it has declining trend (World Bank, 2018).
failure: Sorry that is wrong. Try again.
...
```{r "4_1",fig.width=6, fig.height=4}

#load library
library(ggplot2)
library(ggbreak) 
library(patchwork)
# create data frame of study components
dat=data.frame(Event=c("SMS charcoal survey (A1)", "Charcoal ash collection", "SMS charcoal survey","Loan payments (C1, C2)", "SMS charcoal survey (end)"),
               start=c(-30,0, 0, 0, 365),
               end=c(0,30, 60, 90, 425))

    
# construct the figure 
p=ggplot(dat) + 
geom_segment(aes(x=start,xend=end, y=reorder(Event, -start), yend=Event, color=Event),size=10, show.legend = FALSE)+
#manual x axis breaks
scale_x_continuous(breaks = c(seq(from = -30, to = 90, by = 30), seq(from=365, to=425, by=30)), labels = c("Visit 1", "Visit 2", "Visit 3", "60", "90", "365", "395", "425"))+
#x axis break between 100 and 365 days
scale_x_break(c(100, 365))+
#defining x/y axis label and title
xlab("Time")+
ylab("Event")+
ggtitle("Timeline of the experiment")+
...
#< quiz "SMS survey"
question: After visit 1 starts the SMS charcoal survey for all participants. 

sc:
- True
- False *
success: Great, your answer is correct!
failure: Sorry that is wrong. It begins only for respondents in the attention group 1. Everyone else only get placebo SMS. The SMS surveys for all attendants are starting after visit 2.
#>

#< quiz "treatment goup"
question: Every respondent can only be a member of an attention group or a credit group.
sc:
- True
- False *
success: Great, your answer is correct!
failure: Sorry that is wrong. Every respondent is assigned to an attention treatment group AND a credit group (but one or even both can also only be the respective control group).
#>

#< quiz "jikokoa"
question: At what time study participants get their Jikokoa?
sc:
- Visit 1
- Visit 2 *
- After 90 days
...
#< quiz "jikokoa2"
question: Can every participant buy a Jikokoa within the trial?
sc:
- Yes
- No, it depends on their WTP in conjunction with the random price distribution*
success: Great, your answer is correct!
failure: Sorry that is wrong. Try again!
#>

#< award "Quiz master"
You solved your first quizzes!
#>
...
#>
dat_sms=read_dta("SMS_clean_sms_replication.dta")
head(dat_sms)
...
dat_sms[_,_]
#>
i=which.max(dat_sms$amount_weekly)
dat_sms[i,]
```
...
- `filter():` creates a subset of a given data frame with all the rows satisfying the conditions. You can add conditions by `&` (AND) or `|` (OR).

- `select():` select one or multiple variables of a given data frame or discard them with `-`.

- `group_by():` takes an existing data frame and converts it into a grouped one. Grouping is done by a variable and the following operations are applied to the respective groups.

- `summarize()/summarise():` will contain one column for each grouping variable and one column for each of the summary statistics. For example inside `summarize` you can use functions like `mean(), sum(), min(), max() etc.`.

- `arrange():` Sorts the chosen variables of a data frame. Use `desc()`to sort a variable in descending order.

- `mutate():` can create new columns or modify existing ones.

The first input of each function is the name of the corresponding data frame, which constitutes the basis of the operation. This input can be omitted if the `pipe-operator %>%` is used, but for this matter there will be a special info box later.

For more detailed information about the package see: https://cran.r-project.org/web/packages/dplyr/dplyr.pdf
#>
...
#>
dat_frac=select(dat_main, respondent_id,jikokoa)
filter(dat_frac, respondent_id==name)
...
#< quiz "dplyr"
question: Which `dplyr` function do we need to add a new variable to our data set?
sc:
- select
- filter
- mutate *
...
#>
dat_main=dat_main %>%
mutate(incomeself_weekly=b_incomeself_USD*7) 
head(dat_main)
...
#< quiz "school"
question: How many years does primary school last in Kenya?
answer: 8
#>

#< quiz "school2"
question: How many years does secondary school last in Kenya?
answer: 4
#>
...
#< quiz "income"
question: What is the approximate average **monthly** income of the respondents in USD?
sc:
...
#< quiz "energy cost"
question: What proportion of weekly household income is spent on energy taking the median of both variables?
sc:
...
#< quiz "savings"
question: How many monthly self-incomes the median respondent has accrued as savings?
sc:
...
#< award "Dplyr expert"
You solved all kinds of dplyr exercises!
#>
...
#< quiz "cost"
question: What change in the results do you expect, if we only look at participants who have adopted the Jikokoa?
sc:

- The average coal cost after visit 2 will be lower*
- The average coal cost after visit 2 will be higher
...
#>
dat_main %>%
filter(jikokoa==1 & (a1 == 1 | a2==1))%>%
summarize(coal_cost_weekly_jiko=mean(sms_amount_weekly_pre_USD, na.rm=TRUE), coal_cost_weekly_jikokoa=mean(sms_amount_weekly_post_USD, na.rm=TRUE))
...
With the package `ggplot2`, a large variety of graphics can be created, and it visualizes data in an aesthetically appealing fashion. The basic function as well as the base layer of the package is `ggplot()`. The `ggplot()` function is typically used to initiate the creation of a graphic, where the data source is specified and the visual elements such as the axes and other aesthetic features are defined using the `aes()` argument. Multiple layers can be added to the plot using the addition operator (+).


In the next line you can find a basic template you can use for different types of plots (the plot is defined with `_FUNCTION`):

`ggplot(data = <DATA>, mapping = aes(<MAPPINGS>)) +  <geom_FUNCTION>()`

Additional layers we will use in the next task will now be explained briefly.

- `geom_line()`: represents a line chart

- `geom_point()`: adds a layer of points

- `geom_vline()`: adds a vertical reference line to the plot

- `labs()`: can be used to add a title and the axis labels to the graph

- `scale_color_discretes()`: allows adjustments of the legend of colors 

- `scale_x_continuous/scale_y_continuous`: allows adjustments of x/y scales like setting the locations of the axis tick marks with `break` and define axis limits


In this context also the package `ggthemes` is useful, for enhancing our graphs in a visual manner by adding a theme to them. In this problem set, we will apply `theme_wsj` a Wall-Street Journal theme. It can also be added to the graph like a layer by using a "+".
...
dat_main %>%
group_by(jikokoa)%>%
summarize(Weight_of_bucket= mean(bucket_kg_st, na.rm=TRUE))
```
...
#>
dat_control %>%
summarise(WTP_control=mean(finwtp_USD, na.rm=TRUE))
```
...
#< quiz "WTP"
question: If the treatments have a positive effect, the total average WTP should be...?
sc:
- lower
- higher *
success: Great, your answer is correct!
...
#< quiz "treatment groups"
question: Which treatment leads to the maximum treatment effect, according to the observed averages from above? 
sc:

- Attention treatment
- Credit treatment
- A combination of attention and credit treatment*

success: Great, your answer is correct!
failure: Sorry that is wrong. Try again!
#>

#< quiz "treatment groups2"
question: Which treatment group has a higher treatment effect? 
sc:

- Attention group
- Credit group*
- There is no treatment effect in both groups

success: Great, your answer is correct!
failure: Sorry that is wrong. Try again!
#>

#< quiz "treatment groups3"
question: state the average treatment effect in USD $E(\tau_{i})$ for the credit treatment (0 digits).
answer: 12
roundto: 1
...
dat_main %>%
 mutate(treatment = ifelse(treata_pooled == 0 & treatc_pooled == 0, "0",
                    ...
                    ifelse(treata_pooled == 1 & treatc_pooled == 0, "A",
                    ifelse(treata_pooled == 1 & treatc_pooled == 1,"AC", NA))))) %>%
 ggplot(mapping=aes(x=finwtp_USD, fill=treatment))+
...
#>
dat_main %>%
summarise(BDM_price=mean(price_USD, na.rm=TRUE))
```
...
#< quiz "linear regression beta 0"
question: What measures $ß_0$? 
sc:

- the average cost per week resulting without a Jikokoa*
- the average cost per week resulting with a Jikokoa
- the reduction/increase in weekly charcoal cost if a Jikokoa is adopted

success: Great, your answer is correct!
failure: Sorry that is wrong. Try again!
#>
#< quiz "lin reg beta 1"
question: What measures $ß_1$? 
sc:

- the average cost per week resulting without a Jikokoa
- the average cost per week resulting with a Jikokoa
- the reduction/increase in weekly charcoal cost if a Jikokoa is adopted compared to non-adoption*

success: Great, your answer is correct!
failure: Sorry that is wrong. Try again!
#>

#< quiz "lin reg epislon"
question: What measures $\varepsilon_{it}$? 
mc:

- the error term/disturbance of the regression*
- the distance from data points to the regression line*
- it represents the effect of variables or other influences that were not captured in the equation*
- it should be a random variable with a mean of zero and a constant variance*
...
#>
reg_ols=lm(amount_weekly_USD~jikokoa, dat_reg)
summary(reg_ols)
```

#< quiz "Regression output"
question: Based on the results of the regression analysis, what inferences can be drawn?
mc:

- the reduction of weekly charcoal costs amounts to 1.93 USD if a Jikokoa is adopted*
- the relationship is significant*
- the average weekly charcoal costs using a Jikokoa are 5.71 USD
- there exists a causal relationship
...
Congratulations you have reached the first level of regression expert. You have estimated the effect of adopting the Jikokoa on weekly charcoal expenses using OLS regression.
#>
...
Assume that our true simple model to estimate the weekly charcoal cost is constructed as follows:
$$
amountweekly _{it} = ß _{0} + ß_1jikokoa_{it} + ß_2householdsize_{it} +\varepsilon_{it}
$$
The weekly coal expenditure is not only influenced by the adoption of the Jikokoa stove but also by the size of the corresponding household. Larger households require a greater amount of coal to meet their needs. At the same time, the size of a household also impacts the adoption of the efficient stove. If the needed amount of coal is higher, total possible savings with the Jikokoa increase, making adoption more attractive.

Employing a simple model that only includes the `jikokoa` variable, but neglects to consider `Household_size`, results in an error term that implicitly contains the household size.

$$
\varepsilon_{it} = ß_2householdsize_i +u_{it}
$$
The disturbance is then a function of the Jikokoa variable because `jikokoa` is correlated with `Household_size` and thereby invalidating our independence assumption. The result is a biased estimator of `jikokoa` because the effect of the omitted variable (`Household_size`) is included in the coefficient of the variable with which it is correlated (`jikokoa`).

More information about this matter you can also find in Greene, W. (2020), Chapter 2.
#>
...
The stargazer package provides a powerful tool for presenting clear and concise summaries of multiple regression results, often used in academic publications. The stargazer function allows for customization of the report title, variable labels, and the inclusion of specific statistics such as confidence intervals. Additionally, like ggplot, various templates can be selected to adjust the presentation style of the results. 

More information you can get under the following link: https://cran.r-project.org/web/packages/stargazer/vignettes/stargazer.pdf
#>  
...
reg_control=lm(amount_weekly_USD~jikokoa + Household_size+b_children+  savings_KSH + b_incomeself_KSH+ RiskAverse+ CreditConstrained+d_jikokoalast_years+v1_beliefs_annual_mean+ v1_beliefs_annual_sd +finwtp_USD+price_USD+treata+treatc+sms_amount_weekly_pre,  dat_reg)
stargazer(reg_ols, reg_control, type="text")
```
...
`felm()` is especially helpful for linear group fixed effects, but it also offers an option to cluster robust standard errors. The formula specification of `felm()` is a formula with four parts. The first part consists of explanatory variables similar to `lm()` and the second part consists of factors to be projected out. The third part is an IV specification, which we will use later. The fourth part is a cluster specification for the standard errors `felm(y ~ x1 + x2 | f1 + f2 | (Q|W ~ x3+x4) | cluster1 + cluster2, data frame, exactDOF=FALSE, subset)` where `y` is the dependent variable, `x1,x2` are regressors, `f1,f2` are factors to be projected out, `Q and W` are variables which are instrumented by `x3 and x4`, and `cluster1, cluster2` are factors to be used for computing cluster robust standard errors. Parts that are not used can be specified as 0.

For more information, visit https://cran.r-project.org/web/packages/lfe/lfe.pdf on page 19. 
#>
...
question: Clustered standard errors are? 
sc:

- higher than the standard errors in lm() regression* 
- lower than the standard errors in lm() regression
...
Congratulations you have reached the second level of regression expert. You have estimated the effect of adopting the Jikokoa on weekly charcoal expenses using OLS regression with control variables and clustered standard errors.
...
#< quiz "Regression IV 1"
question: What is your prediction for the correlation between the variables `jikokoa` and `price_USD`? 
sc:
...
#>
cor(dat_main$jikokoa, dat_main$price_USD, use="complete.obs")
```
...
#< quiz "Regression IV 4"
question: If we would regress `jikokoa` on `price_USD` what sign of $\hat\beta_1$ would you expect? 
sc:
...
`felm()` is especially helpful for linear group fixed effects, but it also offers an option to cluster robust standard errors. The formula specification of `felm()` is a formula with four parts. The first part consists of explanatory variables similar to `lm()` and the second part consists of factors to be projected out. The third part is an IV specification, which we will specify in the next task. The fourth part is a cluster specification for the standard errors `felm(y ~ x1 + x2 | f1 + f2 | (Q|W ~ x3+x4) | cluster1 + cluster2, data frame,exactDOF=FALSE, subset)` where `y` is the dependent variable, `x1,x2` are regressors, `f1,f2` are factors to be projected out, `Q and W` are variables which are instrumented by `x3 and x4`, and `cluster1, cluster2` are factors to be used for computing cluster robust standard errors. Parts that are not used can be specified as 0.
...
stargazer(reg_iv, reg_clust, type="text")
``` 
...
Congratulations, you have reached the third and last level of regression expert. You have estimated the effect of adopting the Jikokoa on weekly charcoal expenses using an instrumental regression including control variables and clustered standard errors.
#>
...
#< quiz "Regression IV 2"
question:  Which variable besides the random price could be an appropriate instrument? 
sc:

- The credit treatment dummy variable*
- The attention treatment dummy variable
...
#< quiz "Regression IV 3"
question: Consider the correlation between `jikokoa` and `treatc_pooled`, what do you expect? 
sc:
...
#>
cor(dat_main$jikokoa, dat_main$treatc_pooled, use="complete.obs")
```
...
reg_iv_2=felm(amount_weekly_USD~Household_size + b_children + savings_KSH + b_incomeself_KSH + RiskAverse + CreditConstrained + finwtp_USD + d_jikokoalast_years+ v1_beliefs_annual_mean + v1_beliefs_annual_sd + treata+sms_amount_weekly_pre|0|(jikokoa~price_USD + treatc_pooled)|respondent_id,  dat_reg)

reg_iv=felm(amount_weekly_USD~ Household_size + b_children + savings_KSH + b_incomeself_KSH + RiskAverse + CreditConstrained + finwtp_USD + d_jikokoalast_years+ v1_beliefs_annual_mean + v1_beliefs_annual_sd + treata + treatc + sms_amount_weekly_pre|0|(jikokoa~price_USD)|respondent_id,  dat_reg)
...
For performing IV regressions with `ivreg()` use the following syntax:
`ivreg(dependent_variable ~ endogen variable + controls | controls + instruments)`.

Regressors and instruments in `ivreg()` are most easily specified in a formula with two parts on the right-hand side, like y ~ x1 + x2 | z1 + z2 + z3. Y is the dependent variable, x1 and x2 are the regressors and z1, z2, and z3 are the instruments. Exogenous regressors have to be included as instruments for themselves.

`Summary()` shows the standard regression statistics. If you include the specification `diagnostics=TRUE`, it carries out the corresponding diagnostic tests. These comprise a weak instrument test, the Wu-Hausman test, and a Sargan test. The weak instrument test examines if there is a weak correlation between the endogenous variable and its corresponding instruments. The Wu-Hausmann test tests if the replaced variable is indeed endogenous, and the Sargan test assesses instrument exogeneity.

More information you can find on this page: https://rdrr.io/cran/AER/man/ivreg.html
...
library(AER)
iv_reg=ivreg(amount_weekly_USD~ jikokoa+finwtp_USD+Household_size + b_children + savings_KSH + b_incomeself_KSH + RiskAverse + CreditConstrained+d_jikokoalast_years+ v1_beliefs_annual_mean+ v1_beliefs_annual_sd +treata + sms_amount_weekly_pre| finwtp_USD+ Household_size + b_children + savings_KSH + b_incomeself_KSH + RiskAverse + CreditConstrained+d_jikokoalast_years+v1_beliefs_annual_mean+ v1_beliefs_annual_sd+treata + sms_amount_weekly_pre+ price_USD+treatc_pooled, data=dat_reg)
summary(iv_reg, diagnostics=TRUE)

``` 

#< quiz "Sargan"
question: Does the result of the Sargan test imply that our instruments are exogenous?
sc:
...
$$
#< quiz "Regression credit treatment"
question: What measures $\beta_1$? 
sc:

- The change in WTP in USD provoked through credit treatment, assuming constant control variables*
- The average WTP in USD for respondents with credit treatment
- The average WTP in USD for respondents without credit treatment
...
#> 
reg_credit2=lm(finwtp_USD ~ c1+c2+Household_size + b_children + savings_USD+ b_incomeself_KSH +d_charcoalbuy_KSH+ RiskAverse + CreditConstrained+d_jikokoalast_years+ v1_beliefs_annual_mean+ v1_beliefs_annual_sd, dat_main)

stargazer(reg_credit1, reg_credit2, type="text",omit = c("Household_size", "b_children","savings_USD", "b_incomeself_KSH", "d_charcoalbuy_KSH", "RiskAverse", "CreditConstrained","d_jikokoalast_years", "v1_beliefs_annual_mean", "v1_beliefs_annual_sd", "Constant"))
```

#< quiz "Regression credit treatment 2"
question: Does the credit treatment C1 or C2 generate a higher WTP? 
sc:
...
#> 
reg_att=lm(finwtp_USD ~ a1+a2+Household_size + b_children + savings_USD+ b_incomeself_KSH +d_charcoalbuy_KSH+ RiskAverse + CreditConstrained+d_jikokoalast_years+ v1_beliefs_annual_mean+ v1_beliefs_annual_sd , dat_main)

stargazer(reg_att, type="text",omit = c("Household_size", "b_children","savings_USD", "b_incomeself_KSH", "d_charcoalbuy_KSH", "RiskAverse", "CreditConstrained","d_jikokoalast_years", "v1_beliefs_annual_mean", "v1_beliefs_annual_sd", "Constant"))
```
...
question: Which interaction term must we add to analyze the relationship between credit and attention to loan payments? 
sc:

- Credit (pooled) and A1
- Credit (pooled) and A2*
...
#< quiz "interaction term 2"
question: What is the change in WTP if a respondent is in a credit and a2 treatment group? 
sc:
